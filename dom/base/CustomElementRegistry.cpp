...
NS_IMPL_CYCLE_COLLECTION_TRACE_BEGIN(CustomElementRegistry)
  for (auto iter = tmp->mCustomDefinitions.Iter(); !iter.Done(); iter.Next()) {
    aCallbacks.Trace(&iter.UserData()->mConstructor,
                     "mCusetomDefinitions constructor",
                     aClosure);
                                    ...
                                    CustomElementReaction* aReaction)
{
  ...
  if (!mReactionStack.IsEmpty()) {
    mReactionStack.LastElement().AppendElement(do_GetWeakReference(aElement));
  ...
  context->DispatchToMicroTask(backupQueueProcessingRunnable.forget());
}

void
CustomElementReactionStack::InvokeBackupQueue()
{
  CustomElementReactionStack::InvokeReactions(mBackupQueue);
}
